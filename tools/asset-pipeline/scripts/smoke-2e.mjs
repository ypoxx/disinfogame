// 2e-Smoke: Büro betreten → Wand-Monitor (LAGEBILD) → Lage-Übersicht.
...
const OUT = path.join(RUNS_DIR, 'smoke-2e');
...
for (let i = 0; i < 14; i++) { await page.keyboard.press('Space').catch(() => {}); await page.waitForTimeout(200); }
await clickByText(/fortsetzen/i).catch(() => {});
await page.waitForTimeout(300);
// Büro betreten: F → 7 (Ihr Büro)
await page.keyboard.press('f').catch(() => {});
await page.waitForTimeout(600);
await page.keyboard.press('7').catch(() => {});
await page.waitForSelector('[aria-label="LAGEBILD"]', { timeout: 12000 }).catch(() => {});
await page.waitForTimeout(500);
await shot('01_office.png');
// Wand-Monitor → Lagebild (dispatchEvent umgeht die Berater-Panel-Überlappung im Test)
await page.locator('[aria-label="LAGEBILD"]').dispatchEvent('click').catch(() => {});
await page.waitForTimeout(700);
await shot('02_lagebild.png');
const count = await page.getByText(/LAGEBILD —/).count();

fs.writeFileSync(path.join(OUT, 'console-errors.txt'), errors.join('\n') || '(keine)');
console.log('2e-Smoke fertig. lagebild-title:', count, 'Konsolen-Fehler:', errors.length);
