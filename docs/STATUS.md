...
**Stand:** 2026-09-12 (UX-Zweitdurchgang + Bild-Prüfung; Owner-Entscheidungen 1a/2c/3a/4a/5c/6b
umgesetzt — s. „Offene Punkte aus der Sitzung 2026-09-12"). Davor: 2026-08-23 (PLAN P0–P16 KOMPLETT UMGESETZT) · Alle 17 Punkte aus
...
- ~~**`npm run lint` defekt**~~ — **✅ behoben** (2026-09-12): `.eslintrc.cjs` angelegt, CI-Gate
  (`.github/workflows/gate.yml`) führt tsc · vitest · lint · build. `no-explicit-any` und
  `react-hooks/exhaustive-deps` blockieren seit dem `any`-Durchgang (Owner-Entscheidung 5c);
  offen bleiben 110 kosmetische Warnungen (ungenutzte Variablen).
...
## 📌 Offene Punkte aus der Sitzung 2026-09-12 (Owner-entschieden)

Aus dem UX/UI-Zweitdurchgang und dem vollen Browser-Durchgang. Der Owner hat
1a/2c/3a/4a/5c/6b entschieden und umgesetzt bekommen; was hier steht, ist der
bewusst zurückgestellte Rest.

### 1b — Die sieben Konsequenz-Wirkungen als Mechanik bauen *(zurückgestellt)*
Sieben `effect`-Kennungen auf Konsequenz-Wahlen stehen nur in den Daten; keine
kommt im TypeScript vor. Entschieden wurde **1a**: Jede Wahl trägt jetzt einen
Preis auf der vorhandenen Skala, damit die Freikarte weg ist. Die Mechanik
selbst bleibt offen — sie ist der eigentliche Spaß-Gewinn:

| Kennung | Wahl | gemeinte Wirkung |
|---|---|---|
...
Wache: `src/story-mode/__tests__/wahlWirkungGuard.test.ts` — sie schlägt an,
wenn eine achte tote Kennung dazukommt oder eine der sieben eingebaut wird,
ohne aus der Liste zu wandern. Dazu gehört `cost.political_influence`
(cons_politician_scandal/distance): eine Ressource, die es im Spiel nicht gibt.

### Weitere 21 kostenlose Konsequenz-Wahlen *(nicht entschieden)*
Neben den sieben gibt es 21 Wahlen, die nie einen Preis in den Daten hatten.
Ein Teil ist offenkundig Absicht („Nur beobachten", „Vorsichtig bleiben",
„Enthüllung akzeptieren"). Auffällig sind dagegen die zynischsten Antworten des
Spiels, die nichts kosten — weder Risiko noch Moralgewicht:
`cons_whistleblower/neutralize` (kritisch), `cons_investigation/intimidate`
(severe), `cons_religious_backlash/discredit_leader`. Für ein Lernspiel über die
Kosten von Desinformation ist das eine Owner-Frage, keine Umsetzungsfrage.

### Alexei warnt nie vor Gegenmaßnahmen *(Befund, nicht behoben)*
`analyzeCountermeasures` filterte auf `NewsEvent.type === 'countermeasure'` —
einen Wert, den die Union nicht kennt. Der Filter war IMMER leer; die
Empfehlung hat nie gefeuert. Gegenmaßnahmen laufen in einem eigenen System
(`getActiveCountermeasures()`) und werden nie zu News-Einträgen. Der Zweig ist
jetzt ehrlich leer statt scheinbar aktiv. Ihn an die richtige Quelle zu hängen,
heißt eine Berater-Empfehlung einzuführen, die es faktisch nie gab —
Owner-Entscheidung.

### Schwierigkeit — Messlatte für die ersten Testspieler *(2c)*
Deterministisch gemessen (72 Partien, seed-fest): **56 Siege / 16 Niederlagen**.
Nach Spielweise: rücksichtslos 15/24, vorsichtig 18/24, **zufällig 23/24**. Die
letzte Zahl ist die Messlatte: Wer ohne Plan würfelt, gewinnt fast immer. Der
Owner entscheidet nach der ersten echten Testrunde; bleibt die Zahl so hoch,
ist das der Beleg für „zu leicht". Beide Verlustwege feuern gleich oft (8/8).

### 110 kosmetische Lint-Warnungen *(bewusst stehengelassen, 5c)*
