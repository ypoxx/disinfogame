...
import { StoryModeColors, StoryModeFonts, stampCtaStyle, stampCtaClass, StoryModeWorld } from '../theme';
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', flexDirection: 'column', padding: '16px 20px', overflowY: 'auto', boxSizing: 'border-box' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 4 }}>
          <span style={{ fontFamily: StoryModeFonts.label, fontSize: 16, color: StoryModeWorld.amber, letterSpacing: 2 }}>ZIELGRUPPEN-ANALYSE</span>
        ...
        {showIntro ? (
          ...
          <>
            ...
            {!affordable && (
              <div data-testid="mv-unaffordable" style={{ marginTop: 8, fontSize: 12, color: StoryModeWorld.red }}>Budget zu niedrig ({budget} / {cost}).</div>
            )}
          </>
        ) : (
          <VortestResultView
  ...
  return (
    <button
      ...
      style={{
        ...
        border: `2px solid ${selected ? StoryModeWorld.amber : StoryModeColors.borderLight}`,
  ...
  return (
    <div>
      ...
      <div data-testid="mv-barometer" style={{ marginBottom: 12 }}>
        <Bar label="So klingt es in der Stichprobe" value={result.predictedReception} color={StoryModeWorld.amber} testid="mv-predicted" />
      ...
      <div style={{ fontFamily: StoryModeFonts.label, fontSize: 13, color: StoryModeWorld.amber, letterSpacing: 1, marginBottom: 6 }}>
      ...
      {result.einwand_de && (
        <div data-testid="mv-einwand" style={{ border: '1px solid #3a7acc', background: 'rgba(58,122,204,0.10)', padding: '7px 11px', marginBottom: 14, fontSize: 11, lineHeight: 1.45 }}>
          ...
          {result.einwandReife && result.einwandReife.einsaetze > 0 && (
            <span style={{ color: StoryModeWorld.red }}> (schon {result.einwandReife.einsaetze}× gelaufen — beim {result.einwandReife.patchBei}. Mal steigt die Abwehr).</span>
          )}
        </div>
      )}
  ...
  return (
    ...
    >
      ...
      <span style={{ flex: '0 0 auto', display: 'flex', gap: 5, fontSize: 10 }}>
        ...
        {s.kippNah && !s.geimpft && (
          <span data-testid={`mv-kipp-${s.segmentId}`} style={{ color: StoryModeWorld.red, fontWeight: 700 }} title="steht kurz vorm Umschlagen">
...
function biasStyle(bias: number): { text: string; color: string } {
  if (bias > 0.12) return { text: '↯ Die Stichprobe schmeichelt der Masche — draußen verfängt sie schwächer.', color: StoryModeWorld.red };
