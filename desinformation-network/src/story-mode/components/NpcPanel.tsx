...
export function NpcPanel({
  ...
  const npcList = (
    <div
      className={variant === 'sidebar' ? 'overflow-y-auto' : 'w-1/2 border-r-4 overflow-y-auto'}
  ...
  const npcDetails = (
    <div className={variant === 'sidebar' ? 'p-4 overflow-y-auto' : 'w-1/2 p-6 overflow-y-auto'}>
      {selectedNpc ? (
  ...
  if (variant === 'sidebar') {
    return (
      <div className="flex flex-col h-full" style={{ backgroundColor: StoryModeColors.surface }}>
        ...
        {/* Stacked vertical layout: list then details */}
