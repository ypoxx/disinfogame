 ...
 *
 * §4.1/B1: Raumbild und Halbfigur werden pixel-sauber skaliert (Integer bzw.
 * Stammbruch statt `cover` ≈ ×0,94) — leichter Zuschnitt bzw. Rand-Ausgleich
 * ersetzt den freien Faktor.
 */
import { useRef } from 'react';
import { getBuildingLayout } from './buildingLayout';
import { snapPixelScale } from './pixelScale';
...
export function NpcRoomView({ npcId, mood = 'neutral' }: NpcRoomViewProps) {
  ...
  const figNat = useNaturalSize(figureUrl);
  const bgSnap = usePixelCover(size, bgNat);
  // Halbfigur: Soll ~74 % der Raumhöhe → nächste pixel-saubere Stufe der nativen Höhe.
  const figH = figNat && size.h > 0 ? figNat.h * snapPixelScale((size.h * 0.74) / figNat.h) : null;
      ...
      style={{
        ...
        ...(bgUrl && bgSnap
          ? {
              ...
              backgroundSize: `${bgSnap.width}px ${bgSnap.height}px`,
      ...
      {figureUrl && (
        <img
          ...
          style={{
            ...
            right: '8%',
            bottom: 0,
            // Fallback vor Bildmaß-Kenntnis: bisherige 74 % (ein Frame lang).
            height: figH ?? '74%',
            imageRendering: 'pixelated',
