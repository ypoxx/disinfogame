...
| `ActionQueueWidget.tsx` | Schwebende Aktionen-Warteschlange unten rechts (eigenes Ausführen/Leeren; default-offen verdeckte sie den FEIERABEND-Knopf). Archiviert 2026-07-07 (Owner F-D + PLAN NAV-Entschlackung N3): die Queue lebt an der **Narrativ-Tafel** (angeheftete Karten an Strang-Spuren, AUSSPIELEN/LEEREN); der Tafel-Hotspot im Büro trägt den Angeheftet-Zähler. Rückwärtsgang zusätzlich: Export in `components/index.ts`, Mount in `StoryModeGame.tsx`, `queueCollapsed`/`toggleQueue` in `stores/panelStore.ts` wiederherstellen. | `src/story-mode/components/NarrativeBoard.tsx` (Spuren) + `QueuePinChip.tsx` (Mini-Chip „N angeheftet" → öffnet die Tafel) + Badge am Board-Hotspot (`PlayerOfficeView`) |
| `ComboHintsWidget.tsx` | Schwebendes Gelegenheits-Fenster-Widget unten links (Fortschritt + nächster Schritt + Verfall). Nach T2/L3 (2026-07-08) als **Zettel am Korkbrett** aufgegangen (V6 zu — kein Floating-Widget mehr). | `NarrativeBoard.tsx` (Fenster-Zettel mit Verfallsfaden + „Nächster Schritt"); das Ergebnis-Modal (`ActionFeedbackDialog.tsx`) zeigt Combo-Hinweise weiterhin selbst. |
